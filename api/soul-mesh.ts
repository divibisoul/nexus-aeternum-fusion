...
const declaredCapabilities = () => ['mesh.handshake','mesh.ping','mesh.describe','capability.list','sara.health','sara.cycle','sara.audit','sara.regenerate','sara.state','sara.capabilities','sara.trace','sara.clareira.audit',...N03_AUDIO_CAPABILITIES.map(c=>c.id)];
...
async function callSara(capability:string,payload:unknown,correlationId:string):Promise<unknown>{
  ...
  try{
    ...
    }if(!response.ok)throw new Error('SARA_HTTP_'+response.status);return body;
