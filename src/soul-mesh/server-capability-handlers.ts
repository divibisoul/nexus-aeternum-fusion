...
async function invokeEdgeFunction(functionName: string, body: Record<string, unknown>, message: SoulMeshMessage): Promise<unknown> {
  ...
  try {
    const response = await fetch(`${base.replace(/\/$/, '')}/functions/v1/${functionName}`, {
      method: 'POST',
      headers: { accept: 'application/json', 'content-type': 'application/json', apikey: key, authorization: `Bearer ${key}`, 'x-soul-mesh-correlation-id': message.correlationId, 'x-soul-mesh-source': message.source },
...
const objectInput = (input: unknown): Record<string, unknown> => input && typeof input === 'object' && !Array.isArray(input) ? input as Record<string, unknown> : { input };

const HYBRID_TRANSPORTS = ['soul-mesh/http', 'supabase-realtime', 'supabase-edge-functions', 'local-event-bus'] as const;

export const N03_SERVER_CAPABILITIES: CapabilityHandlers = {
  'mesh.ping': (payload, message) => ({ ok: true, nucleus: 'N03', receivedFrom: message.source, receivedAt: Date.now(), transports: HYBRID_TRANSPORTS, payload }),
  'mesh.describe': () => ({ nucleus: 'N03', protocol: 'soul-mesh/1', status: 'online', capabilities: getN03CapabilityDescriptors(), transports: HYBRID_TRANSPORTS, channels: { inbound: ['N03.IN.N01', 'N03.IN.N02', 'N03.IN.N04', 'N03.IN.N05', 'N03.IN.N06'], outbound: ['N03.OUT.N01', 'N03.OUT.N02', 'N03.OUT.N04', 'N03.OUT.N05', 'N03.OUT.N06'] } }),
  ...
  'mesh-communication': (payload, message) => ({ ok: true, nucleus: 'N03', receivedFrom: message.source, transports: HYBRID_TRANSPORTS, received: payload }),
};
